...
class GridListTests : BaseTest() {
    ...
    fun `grid_list_content_on_first_page`() {
        onView(withText("Lagiacrus")).check(matches(isDisplayed()))
    }
    ...
    fun `grid_list_content_at_last_position`() {
        ...
        onView(withText("Rathalos")).check(matches(isDisplayed()))
    }
    ...
    fun `vertical_scrolling`() {
        onView(withId(R.id.grid_recycler_view))
            .perform(swipeUp())
        onView(withText("Nergigante")).check(matches(isDisplayed()))
    }
